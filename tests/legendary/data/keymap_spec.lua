...
describe('Keymap', function()
    describe('parse', function()
        ...
        it(
            'for description-only keymaps, mode_mappings is assigned to a list of the modes',
            function()
    ...
    describe('apply', function()
        it('applies keymaps', function()
            ...
            assert.are.same(normal_keymap.lhs, ('%st'):format(vim.g.mapleader or '\\'))
            ...
            assert.are.same(visual_keymap.lhs, ('%st'):format(vim.g.mapleader or '\\'))
        ...
        it('should parse vimscript keymaps and set mode and opts correctly', function()
            for cmd, mode_remap in pairs(cmds) do
                for _, args in ipairs(all_arg_combos) do
                    ...
                    local map_cmd = ('%s %s <leader>t :SomeCommand<CR>'):format(cmd, args_formatted)
                    local expected_opts = { remap = mode_remap.remap }
                    ...
                    for _, mode in ipairs(mode_remap.mode) do
                        ...
                        assert.are.same(
                            keymap.mode_mappings[mode].implementation,
                            ':SomeCommand<CR>'
                        )
                    end
                end
