    ...
    require('legendary.extensions').pre_ui_hook(function()
        ...
        for key, mappings in pairs(keymaps) do
            if key == 'view' then -- these apply to all diffview view types
                legendary_keymaps = vim.list_extend(
                    legendary_keymaps,
                    parse_keymaps(mappings, { diffview_is_open })
                )
            elseif key == 'file_panel' then
                legendary_keymaps = vim.list_extend(
                    legendary_keymaps,
                    parse_keymaps(mappings, { filetype = 'DiffviewFiles' })
                )
            elseif key == 'file_history_panel' then
                legendary_keymaps = vim.list_extend(
                    legendary_keymaps,
                    parse_keymaps(mappings, { filetype = 'DiffviewFileHistory' })
                )
            elseif key ~= 'option_panel' and key ~= 'help_panel' and key ~= 'disable_defaults' then
                -- e.g. the layout name will be "diff2_horizontal"
                legendary_keymaps = vim.list_extend(
                    legendary_keymaps,
                    parse_keymaps(mappings, { diffview_type_matches(key) })
                )
            end
        end
        ...
        require('legendary').commands({
            ...
            {
                ':DiffviewToggleFiles',
                description = 'Toggle the files panel of diffview.nvim view',
            },
