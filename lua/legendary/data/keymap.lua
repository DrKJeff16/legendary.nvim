...
---@return Keymap
function Keymap:parse(tbl, builtin)
    vim.validate({
        ['1'] = { tbl[1], { 'string' } },
...
function Keymap:apply()
    if vim.islist(self.mode_mappings) then
...
function Keymap:id()
    return ('%s %s %s'):format(self.keys, table.concat(self:modes(), ','), self.description)
end

function Keymap:frecency_id()
    return ('%s'):format(self.description)
end
...
function Keymap:from_vimscript(vimscript_str, description) -- luacheck: no unused
    ...
    if not ok then
        error(('[legendary.nvim] Error parsing vimscript keymap: %s'):format(cmd_info))
    end
    ...
    if cmd ~= 'map' and cmd ~= 'noremap' then
        mode = { cmd_first_char }
        opts.remap = not vim.startswith(cmd, ('%sno'):format(cmd_first_char))
    elseif cmd_info.bang then
        mode = { 'i', 'c' }
        opts.remap = cmd ~= 'noremap'
    else
        mode = { 'n', 'v', 's', 'o' }
        opts.remap = cmd ~= 'noremap'
    end
    ...
    local rhs =
        vim.trim(table.concat(vim.list_slice(cmd_info.args, idx_of_keys + 1, #cmd_info.args)))
