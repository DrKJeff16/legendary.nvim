...
function ItemList:sort_inplace(opts)
    ...
    if
        ...
        and (
            ...
            or (
                self.items[1]
                == State.itemgroup_history[opts.itemgroup or ItemList.TOPLEVEL_LIST_ID]
            )
        )
    then
        return
    ...
    if
        opts.most_recent_first
        and State.itemgroup_history[opts.itemgroup or ItemList.TOPLEVEL_LIST_ID]
            ~= self.items[1]
    then
