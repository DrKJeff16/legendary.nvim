...
if not ok then
    error(
        'Frecency sorting requires sqlite.lua (https://github.com/kkharji/sqlite.lua) '
            .. tostring(sqlite)
    )
    return
end
...
function M:transaction(t, params)
    Log.trace(
        'Performing SQL transaction with following data: %s',
        vim.inspect({ query = t, params = params })
    )
...
function M:update(item)
    ...
    local entry_id =
        row_id(self:transaction(self.queries.item_get_entries, { where = { item_id = item_id } }))
    ...
    local timestamps =
        self:transaction(self.queries.timestamp_get_all_entries, { where = { item_id = item_id } })
    local trim_at = timestamps[(#timestamps - Config.sort.frecency.max_timestamps) + 1]
    if trim_at then
        self:transaction(
            self.queries.timestamp_delete_before_id,
            { id = trim_at.id, item_id = item_id }
        )
    end
end
